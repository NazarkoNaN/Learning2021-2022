...
--Б)
SELECT  'Аудиторія' as Літерал1, Num as Аудиторія, 'знаходиться на' as Літерал2 ,Floor as Поверх,'поверсі корпусу' as Літерал3, Building as Корпус, 'і має' as Літерал4, Seats as "К-ть місць", ' місць' as Літерал5
FROM ROOM;

--Завдання 2)
SELECT TEACHER.NAME as Викладач, SUBJECT.Name as Предмет,ROOM.Num as АУД, ROOM.Building as КП,SGROUP.Num || ' ' ||SGROUP.Course as "ГРП і ВРХ",Week as Тиждень,Day as ДН,Lesson as Пара,Type as Тип
...
SELECT T1.NAME as Професор, T2.NAME as Доцент
  FROM TEACHER T1
    JOIN TEACHER T2
    ON T1.POST = 'професор' and T2.POST = 'доцент' and (T1.SALARY + T1.COMMISSION) > (T2.SALARY + T2.COMMISSION + 50);
...
SELECT T1.NAME as Професор, T2.NAME as Доцент
  FROM TEACHER T1
    RIGHT JOIN TEACHER T2
    ON T1.POST = 'професор' and T2.POST = 'доцент' and (T1.SALARY + T1.COMMISSION) > (T2.SALARY + T2.COMMISSION + 50);

SELECT T1.NAME as Професор, T2.NAME as Доцент
  FROM TEACHER T1
    FULL JOIN TEACHER T2
    ON T1.POST = 'професор' and T2.POST = 'доцент' and (T1.SALARY + T1.COMMISSION) > (T2.SALARY + T2.COMMISSION + 50);
--Завдання 7)
